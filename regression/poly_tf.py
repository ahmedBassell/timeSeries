...
# read db
db_path = os.path.join(os.path.dirname(__file__), 'db/AirPassengers.csv')

dateparse = lambda dates: pd.datetime.strptime(dates, '%Y-%m')
data = pd.read_csv(db_path, parse_dates=True, index_col='Month',date_parser=dateparse)
# data = data[:100]

# xs = data['Month'].as_matrix().reshape(-1,1)
ys = data['#Passengers'].as_matrix().reshape(-1,1)
...
for interval in range(1,24):
    W3 = tf.Variable(tf.random_normal([1], dtype=tf.float64), name='weight3_%d' % pow_i, dtype=tf.float64)
    Y_pred = tf.add(tf.mul(tf.cos(2*np.pi*X*n_observations*2 /(interval)), W3), Y_pred)
